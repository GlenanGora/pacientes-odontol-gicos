...
switch ($metodo) {
    case 'GET':
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'listar') {
            $query = "SELECT id_tratamiento, nombre_tratamiento, costo_base, descripcion FROM tbl_tratamientos ORDER BY nombre_tratamiento";
    ...
    case 'POST':
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'agregar') {
            ...
            $descripcion = sanear_entrada($data['descripcion']);

            if (!empty($nombre_tratamiento) && is_numeric($costo_base) && $costo_base >= 0) {
                $query = "INSERT INTO tbl_tratamientos (nombre_tratamiento, costo_base, descripcion) VALUES (?, ?, ?)";
                $stmt = $conexion->prepare($query);
                $stmt->bind_param("sds", $nombre_tratamiento, $costo_base, $descripcion);
    ...
    case 'PUT':
        // Lógica para editar un tratamiento (action=editar)
        if (isset($_GET['action']) && $_GET['action'] == 'editar' && isset($_GET['id'])) {
            ...
            if (!empty($nombre_tratamiento) && is_numeric($costo_base) && $costo_base >= 0) {
                $query = "UPDATE tbl_tratamientos SET nombre_tratamiento = ?, costo_base = ?, descripcion = ? WHERE id_tratamiento = ?";
                $stmt = $conexion->prepare($query);
                $stmt->bind_param("sdsi", $nombre_tratamiento, $costo_base, $descripcion, $id_tratamiento);

                if ($stmt->execute()) {
                    echo json_encode(['success' => true, 'message' => 'Tratamiento actualizado con éxito.']);
                } else {
                    http_response_code(500);
                    echo json_encode(['success' => false, 'message' => 'Error al actualizar el tratamiento: ' . $stmt->error]);
                }
                $stmt->close();
            } else {
                http_response_code(400);
